...
def test_production_env_validator_rejects_defaults():
    ...
    env.update({
        ...
        "MFA_ENCRYPTION_KEY": "6eIIKadR_-GqxFKCxP3OI9EmXNN_JPOkcw4M3263ouk=",
    })

    result = subprocess.run(
        [
            sys.executable,
            str(ROOT / "scripts" / "validate_production_env.py"),
        ],
        env=env,
        text=True,
        capture_output=True,
...
def test_production_env_validator_accepts_safe_values():
    ...
    env.update({
        ...
        "DATABASE_URL": "postgresql+psycopg://rcaa:strong@postgres:5432/rcaa",
        "AUTH_SECRET": "x" * 48,
        "AUTH_BOOTSTRAP_TOKEN": "strong-bootstrap",
        "CORS_ORIGINS": "https://app.example.com",
        "TRUST_PROXY_HEADERS": "1",
        "METRICS_AUTH_TOKEN": "metrics-token",
        "MFA_ENCRYPTION_KEY": "6eIIKadR_-GqxFKCxP3OI9EmXNN_JPOkcw4M3263ouk=",
